...
//Core functions
const enforceBoundaryConditions = (grid) => {
    ...
    for (let xi = -2; xi < 2; xi++) {
        for (let yi = -2; yi < 2; yi++) {
            grid[xi + Math.floor((grid.length - 1) / 2)][Math.floor(yi + (grid[0].length - 1) / 2)] = 250;
        }
    }
    grid[Math.floor((grid.length - 1) / 2)][10 + Math.floor((grid[0].length - 1) / 2)] = 0;
};
const calculateLaplacian = (grid, y, x) => {
    let countx = 0;
    let county = 0;
    let laplacian = 0;
    if (y != 0) {
...
const updateLeapFrog = (grid, grid2, switcher) => {
    enforceBoundaryConditions(grid);
    for (let y = 0; y < No_Y; y++) {
        for (let x = 0; x < No_X; x++) {
...
// Display function
const display = (grid, ctx) => {
    for (let y = 0; y < No_Y; y++) {
        for (let x = 0; x < No_X; x++) {
            ctx.fillStyle = `rgb(${grid[y][x]},${grid[y][x] / 3},${255 - grid[y][x]})`;
